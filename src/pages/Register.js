...
function Register(props) {
  ...
  const handleSubmit = (e) => {
    ...
    if (inputs.name.trim().length < 6) {
      newErrors.push('name') // 錯誤陣列新增名字
    }

    const re = /\S+@\S+\.\S+/

    // toLowerCase() 字串轉小寫
    // test() 用来查看表达式与指定的字符串是否匹配。
    if (!re.test(inputs.email.toLowerCase())) {
      newErrors.push('email') // 錯誤陣列新增email
    }
  ...
  return (
    <>
      ...
      <div className="form-group">
        ...
        <div className="invalid-feedback">這裡填錯了，您眼花了</div>
      </div>
      <div className="form-group">
        ...
        />
        <div className="valid-feedback">email正確</div>
        <div className="invalid-feedback">email格式錯了</div>
      </div>
      <div className="form-group">
